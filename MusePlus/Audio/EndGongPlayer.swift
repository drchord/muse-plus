...
public final class EndGongPlayer: NSObject, AVAudioPlayerDelegate {
    ...
    /// Lookup order: Bundle.main `.m4a` → Bundle.main `.wav` → Documents/Sounds `.wav`
    /// (auto-generated by `BowlAudioGenerator` at first launch). Returns nil if all
    /// three miss, in which case `EndGongPlayer.play(...)` falls through to ChimeEngine.
    private func bundleURL(resourceName: String) -> URL? {
        if let url = Bundle.main.url(forResource: resourceName, withExtension: "m4a") {
            return url
        }
